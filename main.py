...
class Game(tk.Frame):
    ...
    def __init__(self):
        ...
        self.main_grid = tk.Frame(
            self, bg=cfg.GRID_COLOR, bd=3, width=100*(dimension+1), height=100*(dimension+1))  
    ...
    def make_GUI(self):
        ...
        for i in range(dimension):
            ...
            for j in range(dimension):
                ...
                cell_number = tk.Label(self.main_grid, bg=cfg.EMPTY_CELL_COLOR)
    ...
    def start_game(self):
        ...
        self.cells[row][col]["frame"].configure(bg=cfg.CELL_COLORS[2])
        
        # Configuring the content,color,font and background
        self.cells[row][col]["number"].configure(
            bg=cfg.CELL_COLORS[2],
            fg=cfg.CELL_NUMBER_COLORS[2],
            font=cfg.CELL_NUMBER_FONTS[2],
            text="2")
    ...
    def update_GUI(self):
        for i in range(dimension):
            for j in range(dimension):
                ...
                if cell_value == 0:
                    self.cells[i][j]["frame"].configure(bg=cfg.EMPTY_CELL_COLOR)
                    self.cells[i][j]["number"].configure(
                        bg=cfg.EMPTY_CELL_COLOR, text="")

                # Else --> Add the GUI of respective value cell
                else:
                    # giving the background according to the cell value
                    self.cells[i][j]["frame"].configure(
                        bg=cfg.CELL_COLORS[cell_value])
                    # giving the foreground according to the cell value
                    self.cells[i][j]["number"].configure(
                        bg=cfg.CELL_COLORS[cell_value],
                        fg=cfg.CELL_NUMBER_COLORS[cell_value],  
                        font=cfg.CELL_NUMBER_FONTS[cell_value],
                        text=str(cell_value))
